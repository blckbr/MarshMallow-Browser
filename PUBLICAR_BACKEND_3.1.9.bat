...
cd /d "%~dp0"
title MarshMallow 3.1.9 - Publicar Backend + IA + Contas

echo.
echo ==============================================================
echo   MARSHMALLOW 3.1.9 - BACKEND + WORKERS AI + CONTAS
echo ==============================================================
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$code=$LASTEXITCODE;" ^
  "$lines ^| Tee-Object -FilePath '..\MARSHMALLOW_3.1.9_DEPLOY.log' ^| ForEach-Object { Write-Host $_ };" ^
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if($h.backendVersion -ne '3.1.9'){throw ('Backend publicado nao e 3.1.9. Recebido: '+$h.backendVersion)};" ^
  "if(-not $h.accountsConfigured){throw 'Binding ACCOUNTS nao foi ativado'};" ^
  "if(-not $h.workersAiConfigured){throw 'Workers AI binding nao foi ativado'};" ^
  "Write-Host ('[OK] Backend: '+$h.backendVersion) -ForegroundColor Green;" ^
  "Write-Host ('[OK] AI provider: '+$h.aiProvider) -ForegroundColor Green;" ^
  "Write-Host ('[OK] Modelo: '+$h.model) -ForegroundColor Green;" ^
  "$ping=Invoke-RestMethod -Uri ($base+'/api/auth/ping?_mm='+[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()) -Method Get -TimeoutSec 30;" ^
  "if(-not $ping.ok){throw 'Autoteste do AccountStore falhou'};" ^
  "Write-Host ('[OK] AccountStore: '+$ping.storage+' / '+$ping.crypto) -ForegroundColor Green;" ^
...
echo Consulte MARSHMALLOW_3.1.9_DEPLOY.log se ele tiver sido criado.
pause
exit /b 1
