...
setlocal
chcp 65001 >nul
title MarshMallow 5.0.1 - Publicacao oficial

for %%I in ("%~dp0.") do set "ROOT=%%~fI"
set "PS1=%~dp0scripts\windows-publish-5.0.1.ps1"
if exist "%PS1%" goto :helper_found
set "PS1=%~dp0windows-publish-5.0.1.ps1"
if exist "%PS1%" (
  for %%I in ("%~dp0..") do set "ROOT=%%~fI"
  goto :helper_found
)

echo [PACOTE_5_COMPLETO] O script windows-publish-5.0.1.ps1 nao foi encontrado.
...
cd /d "%ROOT%"
echo ==============================================================
echo  MARSHMALLOW 5.0.1 - PUBLICACAO OFICIAL
echo ==============================================================
echo.
...
set "RC=%ERRORLEVEL%"
echo.
if not "%RC%"=="0" echo [FALHA/BLOQUEADO] Consulte PUBLICACAO_5.0.1.log.
