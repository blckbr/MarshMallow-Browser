...
setlocal
chcp 65001 >nul
title MarshMallow 5.0.0 - Validar e compilar

for %%I in ("%~dp0.") do set "ROOT=%%~fI"
set "PS1=%~dp0scripts\windows-build-5.0.ps1"
if exist "%PS1%" goto :helper_found
set "PS1=%~dp0windows-build-5.0.ps1"
if exist "%PS1%" (
  for %%I in ("%~dp0..") do set "ROOT=%%~fI"
  goto :helper_found
)

echo [PACOTE_5_COMPLETO] O script windows-build-5.0.ps1 nao foi encontrado.
...
if not "%RC%"=="0" (
  ...
  echo [OK] Build 5.0.0 criado e validado automaticamente.
  echo Agora execute REGISTRAR_SMOKE_5.0.0.bat e teste o navegador no Windows.
)
pause
exit /b %RC%
