...
try {
    ...
    if ($health.backendVersion -ne '3.2.1') { throw ('Backend publicado nao e 3.2.1. Recebido: ' + $health.backendVersion) }
    ...
    if ($ping.registry -ne 'v3.2') { throw ('Registro de contas incorreto: ' + $ping.registry) }
