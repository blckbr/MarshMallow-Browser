...
const siteRoot = 'MarshMallow-Official-Website-5.0.2/site';
if (exists(siteRoot)) {
  ...
  if (exists(versionPath)) {
    ...
    expect(siteVersion.version === VERSION, 'site version.json matches 5.0.2');
  }
  ...
  if (exists(releasePath)) {
    ...
    expect(meta.version === VERSION, 'site release metadata version matches 5.0.2');
...
const counterModule = read('backend/src/download-counter.js');
expect(wrangler.includes('\"DOWNLOAD_COUNTER\"') && wrangler.includes('\"DownloadCounter\"'), 'Cloudflare Durable Object binding for persistent download counter exists');
expect(backend.includes('export class DownloadCounter extends DurableObject') && backend.includes('/api/downloads/count') && backend.includes('/download/windows'), 'Gateway exposes persistent count and tracked Windows download routes');
expect(counterModule.includes('sumLegacyOfficialDownloads') && counterModule.includes('shouldCountDownload'), 'download counter has legacy migration and duplicate/bot guard helpers');
if (exists(`${siteRoot}/assets/download-count.js`)) {
  const counterSite = read(`${siteRoot}/assets/download-count.js`);
  expect(counterSite.includes('marshmallow-gateway.marshmallow-browser-br.workers.dev/api/downloads/count'), 'site counter reads Cloudflare Gateway');
  expect(!counterSite.includes('api.github.com/repos/blckbr/MarshMallow-Browser/releases'), 'site no longer calculates live total directly from GitHub assets');
}
if (exists(`${siteRoot}/assets/download.js`)) {
  const downloadSite = read(`${siteRoot}/assets/download.js`);
  expect(downloadSite.includes('marshmallow-gateway.marshmallow-browser-br.workers.dev/download/windows'), 'site installer button uses tracked Gateway redirect');
}
