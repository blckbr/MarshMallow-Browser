...
$Public=Join-Path $Root 'MarshMallow-GitHub-Public-5.0.0'
...
$Notes=Join-Path $Public '5.0.0.md'
$Log=Join-Path $Root 'PUBLICACAO_5.0.0.log'
...
try{
  ...
  $temp=Join-Path $env:TEMP ("MarshMallow-5.0-publish-"+[Guid]::NewGuid().ToString('N'))
  ...
  try{
    ...
    if($LASTEXITCODE -ne 0){
      Exec 'Commit registro publico 5.0.0' { & git.exe commit -m 'MarshMallow 5.0.0 public release record' }
  ...
  if(-not (Test-GitHubReleaseExists $Tag $Repo)){
    ...
    Exec 'Criar GitHub Release 5.0.0' { & gh.exe release create $Tag $Installer $HashFile --repo $Repo --title "MarshMallow $Version" --notes-file $Notes }
  } else {
    Exec 'Atualizar assets GitHub Release 5.0.0' { & gh.exe release upload $Tag $Installer $HashFile --repo $Repo --clobber }
  }
  ...
  Exec 'Cloudflare Pages deploy' { & npx.cmd wrangler pages deploy $Site --project-name marshmallow-browser-br --commit-message MarshMallow-5.0.0 }
  ...
  Write-Host ''
  Write-Host 'Nenhum passo posterior sera executado. Consulte PUBLICACAO_5.0.0.log.' -ForegroundColor Yellow
  exit 1
}finally{
