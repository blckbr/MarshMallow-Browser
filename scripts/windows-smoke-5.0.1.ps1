...
if (-not (Test-Path $BuildReport) -or -not (Test-Path $Installer)) { throw 'Primeiro execute VALIDAR_E_COMPILAR_MARSHMALLOW_5.0.1.bat.' }
...
Write-Host ''
Write-Host 'MARSHMALLOW 5.0.1 - SMOKE TEST OBRIGATORIO' -ForegroundColor Cyan
Write-Host 'Instale/abra o MarshMallow 5.0.1 gerado e confirme cada item com S somente se ele realmente funcionar.'
Write-Host 'Qualquer N bloqueia a publicacao.' -ForegroundColor Yellow
Write-Host ''

$checks=@(
  'Instalador com MarshMallow aberto: deixe o navegador funcionando e execute novamente MarshMallow-Setup-5.0.1.exe. O navegador deve salvar a sessao, fechar automaticamente e a instalacao deve continuar SEM a caixa tecnica "MarshMallow esta funcionando / Clique OK para fechar". Ao abrir depois, a sessao normal deve permanecer.',
